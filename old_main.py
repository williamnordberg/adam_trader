...
def run_visualize_factors_states(visualization_queue):
    while True:
        # Wait until there is a token in the queue
        token = visualization_queue.get()
        if token == 'START':
            ...
            # After finishing, put 'FINISH' in the queue
            visualization_queue.put('FINISH')


def trading_loop(queue, lock, visualization_queue, long_threshold: float, short_threshold: float,
    ...
    while True:
        ...
        # Before the model goes to sleep, put 'START' in the visualization queue
        visualization_queue.put('START')
        sleep(20 * 60)  # Sleep for 20 minutes

        # When the model wakes up and is ready to run, it checks the visualization_queue.
        # If the queue is not empty, it means the visualization process is running, so it waits.
        while not visualization_queue.empty():
            token = visualization_queue.get()
            if token == 'FINISH':
                break
            sleep(1)  # Sleep for a short period before checking the queue again
...
if __name__ == "__main__":
    ...
    visualization_queue_outer = Queue()

    visualization_charts_process = Process(target=run_visualize_factors_states, args=(visualization_queue_outer,))
    ...
    # Run models
    for i in range(1, 4):
        process = Process(target=trading_loop,
                          args=(queue_outer, lock_outer, visualization_queue_outer, *models[f'model{i}']))
        process.start()
        time.sleep(60)
