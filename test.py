...
def create_layout(fig, fig_trade_result):
    ...
    app.layout = html.Div(style={'backgroundColor': BACKGROUND_COLOR, 'color': TEXT_COLOR}, children=[
        ...
        dcc.Graph(id='trade-results-chart', figure=fig_trade_result, style={'width': '90%', 'height': '100vh'}),
...
def update_graph_live(n):
    ...
    # Add a bar chart for PNL
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=(df["PNL"] / 1000),
                               name='PNL(K)'))

    # Add a bar chart for number_of_long
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=df["long_trades"],
                               name='long trades Number'))

    # Add a bar chart for number_of_short
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=df["short_trades"],
                               name='short trades Number'))

    # Add a bar chart for number_of_trades
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=df["win_trades"],
                               name='win trades'))

    # Add a bar chart for number_of_trades
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=df["loss_trades"],
                               name='loss trades'))

    # Add a bar chart for number_of_trades
    updated_fig.add_trace(go.Bar(x=df['weighted_score_category'], y=df["total_trades"],
                               name='Number of Trades'))

    # Update the layout and show the plot
    updated_fig.update_layout(title='Trade Results',
                            xaxis_title='Model Name',
                            yaxis_title='Value',
                            barmode='stack')
    updated_fig.update_layout(plot_bgcolor=BACKGROUND_COLOR, paper_bgcolor=BACKGROUND_COLOR)

    for annotation in updated_fig['layout']['annotations']:
        annotation['font'] = dict(color=TEXT_COLOR, size=16)  # set color and size of subplot titles
