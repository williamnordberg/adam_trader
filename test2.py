...
def read_gauge_chart_data():
    ...
    fig = make_subplots(rows=4, cols=5,
                        specs=[[{'type': 'indicator'}, {'type': 'xy'}, {'type': 'indicator'}]*4]*5)

    # Add all your gauge plots here
    # For example:
    fig.add_trace(go.Indicator(mode='gauge', value=50, domain={'x': [0, 1], 'y': [0, 1]}), row=1, col=1)
    fig.add_trace(go.Indicator(mode='gauge', value=50, domain={'x': [0, 1], 'y': [0, 1]}), row=1, col=3)
    fig.add_trace(go.Indicator(mode='gauge', value=50, domain={'x': [0, 1], 'y': [0, 1]}), row=1, col=5)

    # Add empty scatter plots to occupy the remaining space
    for i in range(1, 5):
        for j in range(1, 6, 2):
            fig.add_trace(go.Scatter(x=[], y=[], mode='markers'), row=i, col=j)
